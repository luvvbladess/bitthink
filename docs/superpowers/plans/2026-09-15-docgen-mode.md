...
## Task 16: план на тысячи страниц и счёт под $100

**Требование пользователя (дословный смысл):** есть DeepSeek API, подключить его; цель — уложиться в 100 долларов на документацию в 5-7 тысяч страниц.

DeepSeek уже был подключён (`_write_section` отдаёт ему разделы `complexity=complex`, `docgen_router.py`). Деньги оказались не узким местом — узким местом был план.

### Замеры, на которых основана задача

| Что | Значение |
|---|---|
| Элемент плана в JSON | 248 симв. ≈ 85 токенов |
| Потолок вывода `gpt-5.6-terra` (`LEAF_OUTPUT`) | 128 000 токенов |
| Отсюда максимум разделов за один вызов | ~1 500, то есть ~2 250 стр. |
| Нужно разделов на 5 000 / 7 000 стр. | 3 334 / 4 667 |
| Вход одного раздела | ≤ 4 400 токенов |
| Выход одного раздела | ~1 000 токенов |

То есть запрос на 5 000 страниц упирался в оборванный JSON, `_parse_outline_response` возвращал пустой список, и прогон молча откатывался на один раздел «Документ». `MAX_SECTIONS = 5000` был недостижим по построению.

Стоимость прогона (входы и выходы по потолкам, планирование включено):

| Страниц | Доля сложных | Эскалация | Итого |
|---|---|---|---|
| 5 000 | 30% | deepseek-v4-pro | $12.63 |
| 5 000 | 100% | deepseek-v4-pro | $16.15 |
| 5 000 | 100% | gpt-5.6-terra | $52.46 |
| 7 000 | 30% | deepseek-v4-pro | $17.66 |
| 7 000 | 100% | deepseek-v4-pro | $22.59 |
| 7 000 | 100% | gpt-5.6-terra | $73.42 |

Вывод: бюджет держится и без DeepSeek, но только DeepSeek даёт запас (×3-4). Без ключа худший случай — $73 из $100, и трёх повторов по сбоям хватает, чтобы вылезти за бюджет.

### Change A — объём из запроса

`_requested_sections(text)`: «5000 страниц», «5 тыс. страниц», «7к страниц», «5-7 тысяч страниц» (берётся верхняя граница), «300 разделов». Единица обязательна, иначе None и поведение прежнее. Страницы переводятся в разделы через `CHARS_PER_PAGE`/`CHUNK_TARGET_CHARS`, результат зажат в `MAX_SECTIONS`.

### Change B — двухуровневый план

`_plan_document` заменяет `_plan_outline` у обоих вызывающих. До `SINGLE_CALL_SECTION_LIMIT = 150` разделов — один вызов, как раньше. Выше: `_plan_outline(as_chapters=True)` строит главы, затем `_expand_chapter` расписывает каждую параллельно теми же батчами `MAX_PARALLEL_SECTIONS`. Вход вызова на главу намеренно маленький (запрос, глава, `CHAPTER_CATALOG_CHUNKS = 80` заголовков, без превью): таких вызовов сотни.

Обязательные свойства: провал разбора главы не теряет главу (она остаётся одним разделом); `chapters[:chapter_count]` — иначе разогнавшийся планировщик оплачивает вызов на каждую лишнюю главу, а разделы сверх лимита всё равно отрезаются; `Section.chapter` доносит главу до сборки, где `# Глава` печатается один раз при смене.

### Change C — цена до запуска и ошибка API как ошибка

- `_estimate_cost_usd(outline, has_sources)` считает по потолкам и печатается в подтверждении вместе с тем, подключён ли DeepSeek, — режим обещает бюджет, значит должен называть его до того, как потратит.
- Клиенты моделей не бросают исключение на ошибке API, а возвращают её текстом с «❌» (`openai_client.py:687`, `deepseek_client.py:107,208,211`). `_write_section` принимал такой баннер за текст раздела: повтор не срабатывал, раздел считался удачным. Неверное имя модели DeepSeek дало бы 3 350 разделов из сообщений об ошибке и рапорт «Готово». Теперь баннер и пустой ответ поднимают исключение — дальше работает обычный цикл повторов и учёт провалов.

### Change D — прогресс в страницах

**Требование пользователя:** «сделай ещё прогресс бар того, сколько страниц готово сейчас, чтобы юзер понимал».

Прогресс был только в разделах («Раздел 240 из 3350»), а заказывают страницы и ждут часами. `_progress_line` печатает полосу, процент, разделы и — главное — готовые страницы, посчитанные по реально написанным символам, а не по плану: модель, которая пишет короче ожидаемого, видна сразу, а не в конце. Та же полоса в фазе планирования по главам. Итоговая сводка тоже называет объём в страницах.

```
📄 ▰▰▰▰▰▱▱▱▱▱ 54% · раздел 1800 из 3350 · готово ~2430 стр. из ~5025
```

Обновление раз на батч — примерно раз в полминуты на большом прогоне.

### Проверка

`tests/test_docgen.py`: разбор объёма (включая диапазон, отсутствие единицы и зажатие в `MAX_SECTIONS`); оценка стоимости под $100 в худшем случае и её рост от эскалации/объёма; `MAX_CHAPTERS * SECTIONS_PER_CHAPTER == MAX_SECTIONS`; разогнавшийся планировщик не даёт лишних вызовов.

`scripts/selfcheck_docgen.py`: «на 5000 страниц» действительно даёт 3 350 разделов через главы с оценкой $10.38, а прогон поменьше собирает .docx, в котором заголовки глав на месте; баннер ошибки API считается провалом раздела и помечается в документе.

### Осталось открытым

- Имя модели `deepseek-v4-pro` уходит в `api.deepseek.com` как есть, трансляции внутренних ID (как `_MODEL_API_ALIASES` в `openai_client.py`) для DeepSeek нет. Локально ключа нет, проверить живым вызовом не на чем. Если имя не то — теперь это видно как провал разделов, а не как «Готово».
- План строится дважды (в подтверждении и в самом прогоне), поэтому числа в подтверждении — оценка, а не то же самое оглавление. На двухуровневом плане разброс больше, чем был.
